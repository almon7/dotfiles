...
- If the pull is refused, or the tree is dirty, stop and say so rather than reaching for `--rebase`, `--force`, `stash`, or `reset`
— deciding what happens to diverged or uncommitted work is the user's call. If there is no upstream or no network, say that and carry on with the local state;
