...
<!-- Adapted from upstash/context7, skills/find-docs, commit 6836bb4 (2026-09-04).
     Upstream ships this as a SKILL.md; here it is the Context7 reference behind
     the local find-docs skill, so refresh it with
     `curl ... -o agents/skills/find-docs/references/context7.md` and reapply the
     three local edits, or they are silently reverted:
       - this comment,
       - the global `ctx7` that context7/install.sh installs, in place of
         upstream's `npx ctx7@latest`,
       - the deleted "Workflow" and "Authentication" sections, which SKILL.md and
         agents/README.md now own respectively.
     Tracking the skill here rather than letting `ctx7 setup` write into ~/.claude
     keeps it reproducible on a new machine and reviewable in a diff. -->

# Documentation Lookup

Retrieve current documentation and code examples for any library using the Context7 CLI.

Run the `ctx7` command directly:
...
It is installed globally by this machine's dotfiles (`./install.sh context7`),
which is also how it is kept current. If the command is missing, run that
installer rather than reaching for `npx` — tell the user if it is not there.
