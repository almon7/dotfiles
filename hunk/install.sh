...
LABEL=hunk    # the tag log() puts in front of every message
...
case "$(uname -s)" in    # branch on the kernel name
  Darwin|Linux) brew_install hunk ;;    # install or upgrade the formula
  *) log 'Only macOS and Linux are supported.'; exit 1 ;;    # anything else is unsupported
esac

warn_if_shadowed hunk    # point out another hunk earlier on PATH
