...
| Select terminal text | Drag within a tmux pane whose application does not request mouse input; release keeps the highlight without copying |
...
Cmd-U / Cmd-D scrolls up / down by one third of the active pane height, with a minimum of one line. The distance adapts when the pane is resized. History entry, selection clearing, and returning to live output work like Cmd-Up / Cmd-Down. In Neovim Normal mode, Cmd-U / Cmd-D uses the editor's existing Ctrl-u/d behavior, including cursor centering and numeric prefixes.
...
**Status bar.** The session name and windows are centred on a black bar. The window separator adds two spaces between entries. A middle dot separates each window's number and name; the number and dot use muted gray (`#6c7086`) in both active and inactive windows. The active window name has bold lavender text on a soft purple background. When zoom hides other panes, a muted amber `+N` immediately follows the window name and flags: `2·claude+2` means two panes are hidden. The count stays on the black bar outside the purple highlight, appears on active and inactive windows, and disappears when unzoomed. The highlight replaces the active-window `*` marker, and the hidden-pane count replaces `Z`; other window flags remain visible.
